...
def filter_characteristics(white, state):
    knee_frac = (0.00022856304700783622
                 + 0.41420297628235303 * state
                 - 0.42118671936182317 * state**2
                 + 0.42118671936182317 * state**3
                 + 0.892743444761949 * state**4)
    knee_freq = (SAMPLE_RATE/2) * (1 - knee_frac)
    pass_band = (6.02059991328 * math.log(white, 2)
                 + 14.036948327785755 * state
                 - 22.850386042107317 * state**2
                 + 34.797524550144153 * state**3)

    assert knee_freq <= SAMPLE_RATE/2
    assert knee_freq >= 0
